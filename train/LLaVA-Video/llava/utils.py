...
warnings.filterwarnings("ignore", message="[h264 @ 0x.*] mmco: unref short failure")
...
def process_video_with_pyav(video_file, data_args):
    ...
    num_frames_to_sample = num_frames = len(frame_idx)
    frame_time = ",".join([f"{i:.2f}s" for i in frame_time])
    return np.stack([x.to_ndarray(format="rgb24") for x in frames]), video_time, frame_time, num_frames_to_sample
