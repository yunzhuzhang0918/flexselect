...
class ModelArguments:
    ...
    token_selector_path: str = field(default=None)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                # if len(sample["input_ids"]) > 4000:
                #     print(f"Skipping sample {i} due to input_ids length > 4000")
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "token_selector" in tunable_parts:
                for name, param in model.get_token_selector().named_parameters():
                    param.requires_grad_(True)
        # import pdb; pdb.set_trace()
        # for name, param in model.named_parameters():
    ...
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
        ...
        trainer.train()
    trainer.save_state()
    trainer.model.get_token_selector().save_pretrained(training_args.output_dir)
