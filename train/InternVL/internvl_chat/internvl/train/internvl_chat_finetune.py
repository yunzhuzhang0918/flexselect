...
class ModelArguments:
    ...
    token_selector_type: Optional[str] = field(
        default=None,
        metadata={'help': 'Path to a token selector config.'}
    )
    token_selector_path: Optional[str] = field(
        default=None,
        metadata={'help': 'Path to a token selector config.'}
    )
    drop_func_name: Optional[str] = field(
        default=None,
        metadata={'help': 'token selection func'}
    )
    ...
    unfreeze_token_selector: bool = field(
        default=False,
        metadata={'help': 'Set to True to train the token selector. Default is False.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def video_get_item(self, data_item):
        ...
        # else:
        #     pixel_values = load_video_from_frames(
        #         video_path,
        #         num_segments=64
        #     )
        ...
        special_tokens = ''.join(['<image>'.format(i + 1) for i in range(len(pixel_values))])
        data_item['conversations'][0]['value'] = data_item['conversations'][0]['value'].replace('<video>', special_tokens)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                ...
                if not isinstance(e, (UnidentifiedImageError, FileNotFoundError, DECORDError)):
                    traceback.print_exc()
                data_item = json.loads(self.raw_data[i])
...
def my_build_transform(input_size):
...
def load_video(video_path, bound=None, input_size=448, max_num=1, num_segments=32):
    ...
    transform = my_build_transform(input_size=input_size)
...
def load_video_from_frames(folder_path, bound=None, input_size=448, max_num=1, num_segments=32):
    # 获取文件夹中的所有帧文件
    frame_files = sorted([f for f in os.listdir(folder_path) if f.endswith('.jpg')], key=lambda x: int(x.split('.')[0]))
    max_frame = len(frame_files) - 1
    ...
    # frame_indices = np.linspace(0, max_frame - 1, num_segments, dtype=int)
    # 计算 group
    group = max_frame // num_segments
    start_frame = random.randint(0, max_frame - group * (num_segments - 1) - 1)
    frame_indices = [start_frame + i * group for i in range(num_segments)]
    
    for frame_index in frame_indices:
        frame_path = os.path.join(folder_path, frame_files[frame_index])
...
def main():
    ...
    # training_args.lr_scheduler_kwargs = {"num_cycles": 3}
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.token_selector_type = model_args.token_selector_type
        config.model_name_or_path = model_args.model_name_or_path
        config.token_selector_path = model_args.token_selector_path
        # config.token_selector_config.vision_hidden_size = config.vision_config.hidden_size
        # config.token_selector_config.downsample_ratio = config.downsample_ratio
        # config.token_selector_config.attn_implementation = "flash_attention_2"
        # config.token_selector_config._attn_implementation = "flash_attention_2"
        ...
        if model_args.token_selector_path is not None:
            model.load_token_selector(config)
            # import pdb; pdb.set_trace()
            # selector_model = InternVLChatModel.from_pretrained(model_args.token_selector_path)
            # state_dict = selector_model.state_dict()
            # incompatible_keys = model.token_selector.load_state_dict(state_dict, strict=False)
            # print(f"Loaded token selector weights from {model_args.token_selector_pth}. Incompatible keys: {incompatible_keys}")
        # import pdb; pdb.set_trace()
        # for x in model.token_selector.language_model.model.layers:
    ...
    if model_args.unfreeze_token_selector:
        token_selector = model.token_selector
        for k, v in token_selector.named_parameters():
            v.requires_grad = True
        logger.info(f'set token selector to train mode')
    ...
    if training_args.do_train:
        ...
        # del trainer.model.token_selector_teacher
        trainer.model.tokenizer_selector.save_pretrained(training_args.output_dir)
        
        trainer.model.token_selector.save_pretrained(training_args.output_dir)
