...
class NanGradientCallback(TrainerCallback):
    def on_pre_optimizer_step(self, args, state, control, **kwargs):
        ...
        # 如果梯度中存在 NaN，跳过参数更新
        if has_nan:
            print("Gradient contains NaN, skipping parameter update.")
            control.should_step = False  # 跳过 optimizer.step()
            control.should_log = True    # 允许日志记录
...
class ModelArguments:
    ...
    token_selector_path: str = field(default="/mnt/sh/mmvision/home/yunzhuzhang/huggingface/lmms-lab/qwen2-0.5b-ov_layer8")
    pretrain_token_selector_path: str = field(default=None)
...
class LazySupervisedDataset(Dataset):
        ...
        if "{" in data_path and "}" in data_path:
            ...
            with open(data_path, "r") as file:
                ...
                for dataset in datasets:
                    ...
                    rank0_print(f"Loaded {len(cur_data_dict)} samples from {json_path}", type(cur_data_dict))
                    self.list_data_dict.extend(cur_data_dict)
        else:
            ...
            with open(data_path, "r") as file:
                ...
                self.list_data_dict.extend(cur_data_dict)
        # self.list_data_dict = self.list_data_dict[608000:]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                if len(sample["input_ids"]) > 1500:
                    print(f"Skipping sample {i} due to input_ids length > 2000")
                    return self.__getitem__(i + 1)  # 返回下一个样本
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if not os.path.exists(video_file) and not os.path.exists(video_file + ".zip"):
                ...
                if "shareVideoGPTV" in video_file:
                    ...
                    with zipfile.ZipFile(video_file, 'r') as zip_ref:
                        ...
                        for idx in sampled_indices:
                            ...
                            try:
                                with zip_ref.open(frame_path) as frame_file:
        ...
        conversation_ids = self.list_data_dict[i]["conversation_ids"]
        
        teacher_ranks_file = f"/mnt/sh/mmvision/home/yunzhuzhang/eval_dev/data/text_guide_scores_64/{conversation_ids}.pt"
        
        if not os.path.exists(teacher_ranks_file):
            return self._get_item(i + 1)
        teacher_ranks_score = torch.load(teacher_ranks_file)
        if(teacher_ranks_score.shape[1] != data_dict["image"][0][0].shape[0] * 210):
            print(f"Failed to read video file: {video_file}", teacher_ranks_score.shape, data_dict["image"][0][0].shape)
            return self._get_item(i + 1)
        data_dict["id"] = self.list_data_dict[i].get("id", i)
        data_dict["teacher_rank"] = teacher_ranks_score
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, teacher_rank = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "teacher_rank"))
        # print("input_shape:", len(input_ids))
        ...
        batch = dict(input_ids=input_ids, labels=labels.long() if labels.dtype == torch.int32 else labels, attention_mask=input_ids.ne(self.tokenizer.pad_token_id), teacher_rank=teacher_rank)
        ...
        if "prompt" in instances[0]:
            ...
            # batch["tome_layers"] = [[6,13,20,27] for instance in instances]
        return batch
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.token_selector_path is not None:
        overwrite_config["token_selector_path"] = model_args.token_selector_path
...
def train(attn_implementation=None):
    ...
    if model_args.token_selector_path is not None:
        model.get_model().initialize_token_selector(model_args=model_args)
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "token_selector" not in name:
                        param.requires_grad_(True)
            if "token_selector" in tunable_parts:
                for name, param in model.named_parameters():
...
if __name__ == "__main__":
    # wandb.init(project="llava_video_npu", name="max_frames_32_patchsize_729", resume="allow")
    random.seed(1234)
    train()