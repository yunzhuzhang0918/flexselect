...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        rank0_print(llava_cfg)
                        rank0_print(model_path)
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if device_map != "auto" and "npu" not in device_map:
    ...
    # if os.getenv("TOME", "None"):
    #     for name, param in model.named_parameters():
    #         # 如果参数是 k_proj 的权重
    #         if 'self_attn.k_proj' in name:
    #             # 直接找到对应的 sim_proj 权重
    #             sim_proj_name = name.replace('self_attn.k_proj', 'sim_proj_k')  # 假设 sim_proj 与 k_proj 在层中是对应的
    #             # 获取对应的 sim_proj 权重
    #             sim_proj_param = dict(model.named_parameters()).get(sim_proj_name)
    #             # 进行复制
    #             if sim_proj_param is not None:
    #                 print("copying weight from k_proj to sim_proj_k")
    #                 sim_proj_param.data.copy_(param.data)
    #         if 'self_attn.q_proj' in name:
    #             # 直接找到对应的 sim_proj 权重
    #             sim_proj_name = name.replace('self_attn.q_proj', 'sim_proj_q')  # 假设 sim_proj 与 k_proj 在层中是对应的
    #             # 获取对应的 sim_proj 权重
    #             sim_proj_param = dict(model.named_parameters()).get(sim_proj_name)
    #             # 进行复制
    #             if sim_proj_param is not None:
    #                 print("copying weight from q_proj to sim_proj_q")
    #                 sim_proj_param.data.copy_(param.data)
