...
accelerate launch --num_processes 1 --main_process_port 12344 -m lmms_eval \
    --model qwen2_5_vl \
    --model_args "pretrained=models/Qwen2.5-VL-7B-Instruct,fps=2,use_flash_attention_2=True,max_num_frames=1024,use_token_selector=$use_token_selector,token_selector_path=$token_selector_path,token_selector_layer=$token_selector_layer,tkn_budget=$tkn_budget,drop_func_name=$drop_func_name" \
    --tasks videomme,mlvu_dev,longvideobench_val_v,lvbench \
    --batch_size 1 \
    --log_samples \
    --log_samples_suffix qwen2_5vl_7B \
