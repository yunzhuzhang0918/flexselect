...
from .token_selector import token_selection, token_selection_mean
import os
import types
...
class Qwen2_5_VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        # print("Qwen2_5_VLVisionBlock:", attn_implementation)
...
class Qwen2_5_Token_Selector_External(Qwen2_5_VLPreTrainedModel):
    """
    ...
    def __init__(self, config: Qwen2_5_VLConfig):
        ...
        self.layers[-1].mrope_section = config.rope_scaling["mrope_section"]
        self.merger_token_selector = Qwen2_5_VLPatchMerger(
                dim=896,
                context_dim=1280,
...
class Qwen2_5_Token_Selector(Qwen2_5_VLPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Qwen2DecoderLayer`]
    ...
    Args:
        config: Qwen2Config
    """
    def __init__(self, config: Qwen2_5_VLConfig, bigger_model):
        super().__init__(config)
        self._attn_implementation = config._attn_implementation
        ...
        drop_func = globals()[drop_func_name]
        self.layers[-1].token_selection_pe =  types.MethodType(drop_func,  self.layers[-1])
        self.layers[-1].mrope_section = self.config.rope_scaling["mrope_section"]
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        # if kwargs["use_token_selector"] and kwargs["token_selector_path"] != "self":
        # self.merger_token_selector = Qwen2_5_VLPatchMerger(
        #     dim=config.token_selector_hidden_size,
        #     context_dim=config.hidden_size,
        #     spatial_merge_size=config.spatial_merge_size,
        # )
        self.gradient_checkpointing = False
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2_5_VLConfig, layer_idx: int):
        ...
        # print("Qwen2_5_VLDecoderLayer", config._attn_implementation)
        self.self_attn = QWEN2_5_VL_ATTENTION_CLASSES["sdpa"](config, layer_idx)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # t_config = AutoConfig.for_model(**config.token_selector_config)
        # setattr(t_config, "drop_func_name", "token_selection")
        # self.token_selector = Qwen2_5_Token_Selector_External(t_config)
        # self.token_selector = Qwen2_5_Token_Selector_External.from_pretrained(t_config.token_selector_path)
        self.post_init()
    ...
    def compress_vision_embeddings(self, vision_embedding_pos, inputs_embeds, tokens_per_frame, max_frames_per_group, video_grid_thw, position_ids):
        ...
        prefix_ids = torch.ones((1, prefix.shape[1] - 1))
        post_ids = torch.ones((1, post.shape[1] - 1))
        prefix_pos_ids = position_ids[:, :, : total_start]
        # post_pos_ids = position_ids[:, :, total_start+total_tokens:]
        # vis_pos_ids = position_ids[:, :, total_start:total_start+total_tokens]
        ...
        # assert tkn_number % tokens_per_frame == 0, f"tkn_number must be interger scale of tokens_per_frame, but got {tkn_number} and {tokens_per_frame}"
        if tkn_number >= total_tokens:
            return torch.ones(total_tokens).to(inputs_embeds.device, dtype=bool), video_grid_thw
        ...
        for group_idx in range(num_groups):
            ...
            group_t = len(frame_indices)
            video_grid_thw[0][0] = group_t
            
            input_ids = torch.cat([
                prefix_ids,
                torch.full((1, 1), self.config.vision_start_token_id),
                torch.full((1, group_t * tokens_per_frame), self.config.video_token_id),
                torch.full((1, 1), self.config.vision_end_token_id),
                post_ids,
            ], dim=-1)
            
            position_ids, rope_deltas = self.get_rope_index(
                    input_ids.to(dtype=torch.long, device=inputs_embeds.device),
                    None,
                    video_grid_thw,
                    None,
                    None,
            ...
            # group_pos_ids = torch.cat([
            #     vis_pos_ids[:, :, idx*tokens_per_frame:(idx+1)*tokens_per_frame] 
            #     for idx in frame_indices
            # ], dim=2)

            # 构建当前组的完整输入序列
            current_embeds = torch.cat([prefix, group_tokens, post], dim=1)
            # current_pos_ids = torch.cat([prefix_pos_ids, group_pos_ids, post_pos_ids], dim=2)
            # 准备位置信息
            current_vision_pos = [[(total_start, group_tokens.shape[1])]]
           

            # 调用选择器
            _, token_indices = self.token_selector(
                inputs_embeds=current_embeds,
                vision_embedding_pos=current_vision_pos,
                tkn_number=tkn_number,
                position_ids=position_ids
            )
            accumulate_t += tkn_number // tokens_per_frame
            # 转换局部索引到全局索引
        ...
        video_grid_thw[0][0] = accumulate_t 
        # 构建最终序列
        return sorted_indices, video_grid_thw
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if hasattr(self, "token_selector"):
                ...
                # if Mode == "EXTERNAL":
                if isinstance(self.token_selector, Qwen2_5_Token_Selector_External):
                    inputs_embeds_for_token_selector = self.token_selector.embed_tokens(input_ids)
                elif isinstance(self.token_selector, Qwen2_5_Token_Selector):
                    inputs_embeds_for_token_selector = inputs_embeds
                ...
                if pixel_values_videos is not None:
                    ...
                    start = torch.where(mask == True)[1][0].item()
                    length = mask.sum().item()
                    vision_embedding_pos = [[[start, length]]]
                    mask_unsqueezed = mask.unsqueeze(-1)
                    ...
                    if video_embeds_for_token_selector is not None:
                        mask_expanded = mask_unsqueezed.expand_as(inputs_embeds_for_token_selector)
                        video_mask = mask_expanded.to(inputs_embeds_for_token_selector.device)
                        video_embeds_for_token_selector = video_embeds_for_token_selector.to(inputs_embeds.device, inputs_embeds.dtype)
                        inputs_embeds_for_token_selector = inputs_embeds_for_token_selector.masked_scatter(video_mask, video_embeds_for_token_selector)
                    else:
                        inputs_embeds_for_token_selector = inputs_embeds
        ...
        if os.getenv("TOKEN_SELECTOR_MODEL", None):
            ...
            # query_start_pos = torch.where(input_ids == 151665)[1]
            # if len(query_start_pos) != 0:
            #     inputs_embeds_for_token_selector = torch.cat([inputs_embeds_for_token_selector[:, :start +length + 1, :], inputs_embeds_for_token_selector[:, query_start_pos.item()+1:, :]], dim=1)
            
            token_index, video_grid_thw = self.compress_vision_embeddings(
                vision_embedding_pos=vision_embedding_pos,
                tokens_per_frame=token_per_frame,
            ...
            video_embeds = inputs_embeds[:, start: start+length, :]
            post_embeddings = inputs_embeds[:, start+length:, :]
            video_embeds = video_embeds[:, token_index, :]
            inputs_embeds = torch.cat([prefix_embeddings, video_embeds, post_embeddings], dim=1)

            # prefix_position_ids = input_ids[:, :start].to(video_embeds.device)
            # vision_position_ids = input_ids[:, start:start+video_embeds.shape[1]].to(video_embeds.device)
            # post_position_ids = input_ids[:, start+length:].to(video_embeds.device)
            # inputs_embeds = torch.cat([prefix_embeddings, video_embeds, post_embeddings], dim=1)
            # input_ids = torch.cat([prefix_position_ids, vision_position_ids, post_position_ids], dim=1)
            # import pdb; pdb.set_trace()
            prefix_position_ids = position_ids[:, :,  :start].to(video_embeds.device)
            vision_position_ids = position_ids[:, :, start: start+length].to(video_embeds.device)
            post_position_ids = position_ids[:, :, start+length:].to(video_embeds.device)
            selected_position_ids = vision_position_ids[:, :, token_index]
            position_ids = torch.cat([prefix_position_ids, selected_position_ids, post_position_ids], dim=2)
